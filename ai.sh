...
PARTCMD="/usr/bin/parted -s /dev/sda -a optimal unit mb"
...
partition ()
{
    ...
    echo "8GB   | swap"
    echo "rest  | /"
    $PARTCMD mklabel msdos
    $PARTCMD mkpart primary ext4 1 513
    $PARTCMD set 1 boot on
    $PARTCMD mkpart primary linux-swap 513 8513
    $PARTCMD mkpart primary ext4 8513 100%
...
prepare ()
{
    echo "executing chroot commands now..."

    # set our locale
    echo "generating locale"
    $CHROOT /usr/bin/sed -i "s/\#$LOCALE/$LOCALE/g" /etc/locale.gen
    $CHROOT /usr/bin/locale-gen
    $CHROOT /bin/echo "LANG=$LOCALE" > /etc/locale.conf
}
